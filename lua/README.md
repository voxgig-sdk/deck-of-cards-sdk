...
The Lua SDK for the DeckOfCards API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Deck(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DeckOfCards(nil):load(
  { id = "test01" }, nil
)
