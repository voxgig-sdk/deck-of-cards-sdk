...
local PileListEntity = {}
PileListEntity.__index = PileListEntity


function PileListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PileListEntity)
  self._name = "pile_list"
  self._client = client
...
function PileListEntity:get_name()
  return self._name
end


function PileListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PileListEntity.new(self._client, opts)
end
...
function PileListEntity:mark_deleted()
  self._deleted = true
end


function PileListEntity:deleted()
  return true == self._deleted
end


function PileListEntity:data_set(args)
  if args ~= nil then
...
function PileListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PileListEntity:match_set(args)
  if args ~= nil then
...
function PileListEntity:match_get()
...
function PileListEntity:stream(action, args, callopts)
...
function PileListEntity:_run_op(ctx, post_done)
  local utility = self._utility
