...
local PileEntity = {}
PileEntity.__index = PileEntity


function PileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PileEntity)
  self._name = "pile"
  self._client = client
...
function PileEntity:get_name()
  return self._name
end


function PileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PileEntity.new(self._client, opts)
end
...
function PileEntity:mark_deleted()
  self._deleted = true
end


function PileEntity:deleted()
  return true == self._deleted
end


function PileEntity:data_set(args)
  if args ~= nil then
...
function PileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PileEntity:match_set(args)
  if args ~= nil then
...
function PileEntity:stream(action, args, callopts)
...
---@param reqmatch PileLoadMatch
---@param ctrl? table
---@return Pile
---@return string? err
function PileEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PileEntity:_run_op(ctx, post_done)
  local utility = self._utility
