...
local ReturnEntity = {}
ReturnEntity.__index = ReturnEntity


function ReturnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReturnEntity)
  self._name = "return"
  self._client = client
...
function ReturnEntity:get_name()
  return self._name
end


function ReturnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReturnEntity.new(self._client, opts)
end
...
function ReturnEntity:mark_deleted()
  self._deleted = true
end


function ReturnEntity:deleted()
  return true == self._deleted
end


function ReturnEntity:data_set(args)
  if args ~= nil then
...
function ReturnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReturnEntity:match_set(args)
  if args ~= nil then
...
function ReturnEntity:match_get()
...
function ReturnEntity:stream(action, args, callopts)
...
---@param reqmatch ReturnLoadMatch
---@param ctrl? table
---@return Return
---@return string? err
function ReturnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReturnEntity:_run_op(ctx, post_done)
  local utility = self._utility
