...
local DrawEntity = {}
DrawEntity.__index = DrawEntity


function DrawEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DrawEntity)
  self._name = "draw"
  self._client = client
...
function DrawEntity:get_name()
  return self._name
end


function DrawEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DrawEntity.new(self._client, opts)
end
...
function DrawEntity:mark_deleted()
  self._deleted = true
end


function DrawEntity:deleted()
  return true == self._deleted
end


function DrawEntity:data_set(args)
  if args ~= nil then
...
function DrawEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DrawEntity:match_set(args)
  if args ~= nil then
...
function DrawEntity:stream(action, args, callopts)
...
function DrawEntity:_run_op(ctx, post_done)
  local utility = self._utility
