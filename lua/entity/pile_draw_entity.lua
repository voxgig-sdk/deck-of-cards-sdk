...
local PileDrawEntity = {}
PileDrawEntity.__index = PileDrawEntity


function PileDrawEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PileDrawEntity)
  self._name = "pile_draw"
  self._client = client
...
function PileDrawEntity:get_name()
  return self._name
end


function PileDrawEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PileDrawEntity.new(self._client, opts)
end
...
function PileDrawEntity:mark_deleted()
  self._deleted = true
end


function PileDrawEntity:deleted()
  return true == self._deleted
end


function PileDrawEntity:data_set(args)
  if args ~= nil then
...
function PileDrawEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PileDrawEntity:match_set(args)
  if args ~= nil then
...
function PileDrawEntity:match_get()
...
function PileDrawEntity:stream(action, args, callopts)
...
---@param reqmatch PileDrawListMatch
---@param ctrl? table
---@return PileDraw[]
---@return string? err
function PileDrawEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PileDrawEntity:_run_op(ctx, post_done)
  local utility = self._utility
