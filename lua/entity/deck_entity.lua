...
local DeckEntity = {}
DeckEntity.__index = DeckEntity


function DeckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeckEntity)
  self._name = "deck"
  self._client = client
...
function DeckEntity:get_name()
  return self._name
end


function DeckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeckEntity.new(self._client, opts)
end
...
function DeckEntity:mark_deleted()
  self._deleted = true
end


function DeckEntity:deleted()
  return true == self._deleted
end


function DeckEntity:data_set(args)
  if args ~= nil then
...
function DeckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeckEntity:match_set(args)
  if args ~= nil then
...
function DeckEntity:stream(action, args, callopts)
...
---@param reqmatch DeckLoadMatch
---@param ctrl? table
---@return Deck
---@return string? err
function DeckEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DeckEntity:_run_op(ctx, post_done)
  local utility = self._utility
