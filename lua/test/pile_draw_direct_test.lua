...
describe("PileDrawDirect", function()
  it("should direct-list-pile_draw", function()
    local setup = pile_draw_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-pile_draw", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"deck01", "pile_name01"}) do
...
function pile_draw_direct_setup(mockres)
