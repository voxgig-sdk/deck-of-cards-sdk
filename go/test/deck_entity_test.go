...
func TestDeckEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DECKOFCARDS_TEST_DECK_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func deckBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DECKOFCARDS_TEST_DECK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DECKOFCARDS_TEST_DECK_ENTID"])
