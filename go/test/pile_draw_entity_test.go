...
func TestPileDrawEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.PileDraw(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.PileDraw(nil).Stream("list", nil, nil) {
...
func pile_drawBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DECK_OF_CARDS_TEST_PILE_DRAW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DECK_OF_CARDS_TEST_PILE_DRAW_ENTID"])
