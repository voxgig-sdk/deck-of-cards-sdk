...
import (
	...
	omni "github.com/voxgig-sdk/deck-of-cards-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DeckOfCardsError); is {
				out := map[string]any{
					"name":    "DeckOfCardsError",
...
func omniCtx(arg any, client *sdk.DeckOfCardsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DeckOfCardsSDK); is {
			client = live
