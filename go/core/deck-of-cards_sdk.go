...
func (sdk *DeckOfCardsSDK) OptionsMap() map[string]any {
...
func (sdk *DeckOfCardsSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *DeckOfCardsSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *DeckOfCardsSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DeckOfCardsSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *DeckOfCardsSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *DeckOfCardsSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("DeckOfCardsSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *DeckOfCardsSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DeckOfCardsSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("DeckOfCardsSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
