...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DECKOFCARDS_TEST_LIVE ||
    'TRUE' === process.env.DECKOFCARDS_TEST_OVERRIDE
  ) {
  ...
  m.DECKOFCARDS_TEST_EXPLAIN = process.env.DECKOFCARDS_TEST_EXPLAIN || m.DECKOFCARDS_TEST_EXPLAIN
