...
//     (\`new DeckOfCardsSDK(...)\` / \`DeckOfCardsSDK.test(...)\` become
//     \`DeckOfCardsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"deck":{"test01":{"id":"test01"}},"draw":{"test01":{"id":"test01"}},"pile":{"test01":{"id":"test01"}},"pile_draw":{"test01":{"id":"test01"}},"pile_list":{"test01":{"id":"test01"}},"return":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DeckOfCardsSDK(...)\` and any
// \`DeckOfCardsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DeckOfCardsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DeckOfCardsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
