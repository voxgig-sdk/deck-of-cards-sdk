...
describe('DrawDirect', async () => {
  ...
  test('direct-list-draw', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-draw', setup.live)) return
    if (skipIfMissingIds(t, setup, ["deck01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DECK_OF_CARDS_TEST_DRAW_ENTID']
