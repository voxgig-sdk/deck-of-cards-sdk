...
describe('PileListDirect', async () => {
  ...
  test('direct-load-pile_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-pile_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["deck_id01","pile_name01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DECK_OF_CARDS_TEST_PILE_LIST_ENTID']
