...
describe('PileDrawDirect', async () => {
  ...
  test('direct-list-pile_draw', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-pile_draw', setup.live)) return
    if (skipIfMissingIds(t, setup, ["deck01","pile_name01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DECK_OF_CARDS_TEST_PILE_DRAW_ENTID']
