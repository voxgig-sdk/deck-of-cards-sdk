...
[MCP](https://modelcontextprotocol.io) server exposing the DeckOfCards SDK as
two agent tools — `deck-of-cards_list` and `deck-of-cards_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/deck-of-cards-mcp)
make build

# 2. Provide credentials via the environment
export DECK_OF_CARDS_APIKEY=sk_live_xxx
...
```jsonc
// deck-of-cards_list: first page of records
{ "entity": "draw" }
{ "entity": "draw", "query": { } }

// deck-of-cards_load: one record by id
{ "entity": "deck", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/deck-of-cards-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user deck-of-cards \
     -- "$PWD"/dist/*/deck-of-cards-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `deck-of-cards_list` and `deck-of-cards_load` tools now appear
   in new sessions. Ask the agent to *"list draw using deck-of-cards"*
   and it calls `deck-of-cards_list` with `{"entity":"draw"}`.
...
```sh
export DECK_OF_CARDS_APIKEY=sk_live_xxx            # API key
export DECK_OF_CARDS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `deck-of-cards_list` tool
...
### Call the `deck-of-cards_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `deck-of-cards_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `deck-of-cards_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DECK_OF_CARDS_APIKEY` | API key sent with every request. |
| `DECK_OF_CARDS_BASE` | Optional override of the API base URL. |
