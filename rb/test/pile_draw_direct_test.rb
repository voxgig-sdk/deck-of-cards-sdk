...
class PileDrawDirectTest < Minitest::Test
  def test_direct_list_pile_draw
    setup = pile_draw_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-pile_draw", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["deck01", "pile_name01"].each do |_live_key|
...
def pile_draw_direct_setup(mockres)
