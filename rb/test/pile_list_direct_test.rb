...
class PileListDirectTest < Minitest::Test
  def test_direct_load_pile_list
    setup = pile_list_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-pile_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def pile_list_direct_setup(mockres)
