...
class ReturnEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DECKOFCARDS_TEST_RETURN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    return_ref01_data_dt0_loaded, err = return_ref01_ent.load(return_ref01_match_dt0, nil)
    assert_nil err
...
def return_basic_setup(extra)
  ...
  entid_env_raw = ENV["DECKOFCARDS_TEST_RETURN_ENTID"]
