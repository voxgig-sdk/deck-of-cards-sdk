...
class ReturnDirectTest < Minitest::Test
  def test_direct_load_return
    setup = return_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-return", setup[:live] ? "live" : "unit")
    if _should_skip
...
def return_direct_setup(mockres)
