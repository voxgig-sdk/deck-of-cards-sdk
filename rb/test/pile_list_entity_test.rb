...
class PileListEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DECKOFCARDS_TEST_PILE_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    pile_list_ref01_data_dt0_loaded, err = pile_list_ref01_ent.load(pile_list_ref01_match_dt0, nil)
    assert_nil err
...
def pile_list_basic_setup(extra)
  ...
  entid_env_raw = ENV["DECKOFCARDS_TEST_PILE_LIST_ENTID"]
