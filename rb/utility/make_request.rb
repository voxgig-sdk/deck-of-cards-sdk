...
module DeckOfCardsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DeckOfCardsResponse.new({})
    ...
    if fetch_err
      ...
      response = DeckOfCardsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DeckOfCardsResponse.new(fetched)
    else
