...
class PileListEntity
  ...
  def make
    opts = @_entopts.dup
    PileListEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DeckOfCardsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PileList, Hash] the current PileList data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DeckOfCardsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PileList fields)
  def match_get
  ...
  #
  # @param reqmatch [PileListLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PileList.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PileList, Hash] the loaded PileList; raises DeckOfCardsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DeckOfCardsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
