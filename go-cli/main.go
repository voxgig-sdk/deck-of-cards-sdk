// Command DeckOfCards-cli is a CLI + REPL for the DeckOfCards SDK,
...
const prompt = "deck-of-cards"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "deck draw pile pile_draw pile_list return"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DECK_OF_CARDS_APIKEY (api key), DECK_OF_CARDS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DECK_OF_CARDS_APIKEY carries the API key and
	// DECK_OF_CARDS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DECK_OF_CARDS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DECK_OF_CARDS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDeckOfCardsSDK(opts)
