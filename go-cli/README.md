...
boru-driven command-line client **and** interactive REPL for the DeckOfCards
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/deck-of-cards-cli)
make build

# 2. See usage (words, entities, env vars)
./deck-of-cards-cli --help

# 3. Provide credentials once, via the environment
export DECK_OF_CARDS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./deck-of-cards-cli load 1 deck            # {id:1} shorthand
./deck-of-cards-cli load '{id:1}' deck       # explicit match map

# 5. Override the API base URL for a single call
DECK_OF_CARDS_BASE=https://api.example.com ./deck-of-cards-cli load 1 deck

# 6. No arguments -> interactive REPL
./deck-of-cards-cli
deck-of-cards> load 1 deck
deck-of-cards> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/deck-of-cards-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/deck-of-cards-cli load 1 deck
   ```
...
```sh
./deck-of-cards-cli load 1 deck          # scalar shorthand for {id:1}
./deck-of-cards-cli load '{id:1}' deck     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `deck-of-cards>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `deck`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/deck-of-cards-cli`. |
...
(`./deck-of-cards-cli <expr>`) and interactively (the REPL), and expressions compose the
