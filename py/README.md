...
The Python SDK for the DeckOfCards API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Deck(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.DeckOfCards(None).load(
    {"id": "test01"}, None
)
