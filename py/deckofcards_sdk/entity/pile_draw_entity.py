...
from deckofcards_sdk.deckofcards_types import (
    PileDraw,
    PileDrawListMatch,
)
...
class PileDrawEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PileDrawEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PileDraw]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PileDraw().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
