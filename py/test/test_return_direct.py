...
class TestReturnDirect:
    ...
    def test_should_direct_load_return(self):
        setup = _return_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-return", "live" if setup["live"] else "unit")
        if _skip:
...
def _return_direct_setup(mockres):
