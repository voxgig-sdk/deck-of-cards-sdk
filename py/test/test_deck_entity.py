...
class TestDeckEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Deck(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _deck_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "deck." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DECKOFCARDS_TEST_DECK_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        deck_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.deck")))
        deck_ref01_data = None
        if len(deck_ref01_data_raw) > 0:
            deck_ref01_data = helpers.to_map(deck_ref01_data_raw[0][1])

        # LOAD
        deck_ref01_ent = client.Deck(None)
        deck_ref01_match_dt0 = {}
        deck_ref01_data_dt0_loaded, err = deck_ref01_ent.load(deck_ref01_match_dt0, None)
        assert err is None
        assert deck_ref01_data_dt0_loaded is not None



def _deck_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/deck/DeckTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DECKOFCARDS_TEST_DECK_ENTID"))
