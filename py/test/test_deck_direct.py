...
class TestDeckDirect:

    def test_should_direct_load_deck(self):
        setup = _deck_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-deck", "live" if setup["live"] else "unit")
        if _skip:
...
def _deck_direct_setup(mockres):
