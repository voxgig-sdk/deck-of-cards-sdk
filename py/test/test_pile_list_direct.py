...
class TestPileListDirect:

    def test_should_direct_load_pile_list(self):
        setup = _pile_list_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-pile_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _pile_list_direct_setup(mockres):
