...
class TestPileDirect:

    def test_should_direct_load_pile(self):
        setup = _pile_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-pile", "live" if setup["live"] else "unit")
        if _skip:
...
def _pile_direct_setup(mockres):
