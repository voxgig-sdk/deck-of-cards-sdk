...
class DeckOfCardsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DeckOfCardsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DeckOfCardsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DeckOfCardsTestRunner.getenv("DECK_OF_CARDS_TEST_LIVE")
        override = DeckOfCardsTestRunner.getenv("DECK_OF_CARDS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DeckOfCardsTestRunner.getenv(key)
        ...
        explain = DeckOfCardsTestRunner.getenv("DECK_OF_CARDS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DeckOfCardsTestRunner._test_control is not None:
            return DeckOfCardsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DeckOfCardsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DeckOfCardsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DeckOfCardsTestRunner.load_test_control()
...
def load_env_local():
    DeckOfCardsTestRunner.load_env_local()


def env_override(m):
    return DeckOfCardsTestRunner.env_override(m)


def entity_data(v):
    return DeckOfCardsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DeckOfCardsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DeckOfCardsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DeckOfCardsTestRunner.load_test_control()


def live_client_options():
    return DeckOfCardsTestRunner.live_client_options()


def live_delay_ms():
    return DeckOfCardsTestRunner.live_delay_ms()
