...
// DeckOfCards SDK PileList entity
...
class PileListEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PileListEntity($this->_client, $opts);
    }

    /**
     * @param PileList|array $args PileList data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DeckOfCardsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PileList|array The current PileList data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PileList fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DeckOfCardsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PileList fields).
     */
    ...
    /**
     * Load a single PileList.
     *
     * @param PileListLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PileListLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PileList|array The loaded PileList as an assoc-array at the
     *   SDK boundary; throws DeckOfCardsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DeckOfCardsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
