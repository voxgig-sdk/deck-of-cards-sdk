...
class FtCtrl extends DeckOfCardsControl
{
...
class FtClient
{
    ...
    private DeckOfCardsUtility $utility;

    public function __construct(DeckOfCardsUtility $utility, array $options)
    {
    ...
    public function get_utility(): DeckOfCardsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DeckOfCardsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DeckOfCardsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DeckOfCardsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DeckOfCardsConfig::make_feature($name);
    ...
    private static function buildUrl(DeckOfCardsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DeckOfCardsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DeckOfCardsResponse($fetched);
        }
    ...
    private function fail(DeckOfCardsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DeckOfCardsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
